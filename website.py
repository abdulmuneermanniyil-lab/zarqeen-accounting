...
class Distributor(db.Model):
    ...
    last_level_check = db.Column(db.DateTime, default=datetime.utcnow)
...
class License(db.Model):
    ...
    commission_earned = db.Column(db.Float, default=0.0) # Track earnings per sale
...
def send_brevo_email(to_email, subject, html_content):
    if not BREVO_API_KEY:
        print(">>> BREVO KEY MISSING. Email logged.")
        return False
...
def check_distributor():
    try:
        ...
        if dist and dist.is_verified:
            # Use dynamic discount based on level
...
def verify_payment():
    ...
    try:
        ...
        new_lic = License(
            ...
            amount_paid=order['amount'] / 100, commission_earned=commission, distributor_id=dist_obj.id if dist_obj else None
        )
...
def register_distributor():
    ...
    if dist and dist.is_verified: return jsonify({'success': False, 'message': 'Email already registered.'})
...
def api_get_distributor_data():
    ...
    # Progress
    curr = dist.level
    target = LEVELS[min(curr+1, 3)]['target']
    month_sales = License.query.filter(License.distributor_id==dist.id, License.created_at >= datetime.utcnow().replace(day=1)).count()
    ...
    return jsonify({
        ...
        "discount": LEVELS[curr]['discount'],
        "commission_pct": LEVELS[curr]['commission'],
        ...
        "progress": {"current_level": LEVELS[curr]['name'], "month_sales": month_sales, "target": target, "next_level": LEVELS[min(curr+1, 3)]['name'], "is_max": curr==3}
    })
...
def add_distributor():
    try:
        ...
        new_dist = Distributor(
            ...
            is_verified=True, level=1 # Admin created always verified
        )
...
def edit_license(id):
    ...
    lic.is_used = (request.form.get("status") == "used")
    db.session.commit()
    return redirect(url_for("dashboard"))
