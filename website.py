...
def add_distributor():
    try:
        ...
        if not code or not email:
            flash("Missing fields", "danger"); return redirect(url_for("dashboard"))
        ...
        new_dist = Distributor(
            ...
            discount_percent=int(request.form.get("discount", 10)),
...
def edit_distributor(id):
    ...
    try:
        ...
        man_pay = request.form.get("manual_paid_total")
        
        if add_pay and safe_float(add_pay) > 0: 
            dist.commission_paid = safe_float(dist.commission_paid) + safe_float(add_pay)
        elif man_pay and man_pay.strip(): 
            dist.commission_paid = safe_float(man_pay)
...
def reset_with_otp():
    ...
    if not dist.otp_code or not dist.otp_expiry:
        return jsonify({"success": False, "message": "No OTP Request"})
