...
def dashboard():
    ...
    settings = Settings.query.first()
    # ⚠️ FIX: Create settings if missing to prevent 500 Error
    if not settings: 
        settings = Settings()
...
def api_distributor_login():
    ...
    if dist and dist.check_password(data.get('password')):
        ...
        check_level_update(dist); dist.api_token = secrets.token_hex(16); db.session.commit(); return jsonify({'success': True, 'token': dist.api_token})
...
def api_get_distributor_data():
    ...
    # ⚠️ FIX: Ensuring all keys exist even if null to prevent JS crash
    return jsonify({
