...
def check_distributor():
    try:
        ...
        if dist and dist.is_verified:
            return jsonify({'valid': True, 'discount': LEVELS[dist.level]['discount'], 'name': dist.name})
...
def create_order():
    try:
        ...
        if code:
            dist = Distributor.query.filter_by(code=code).first()
            if dist and dist.is_verified:
                final_amount = int(base_amount - ((base_amount * LEVELS[dist.level]['discount']) / 100))
...
def add_distributor():
    try:
        ...
        new_dist = Distributor(code=code, name=request.form.get("name"), phone=request.form.get("phone"), email=email, is_verified=True, level=1)
...
def export_data(type):
    ...
    if type == 'licenses':
        cw.writerow(['Date', 'Key', 'Plan', 'Amount', 'Distributor', 'Status', 'Ver', 'Expiry'])
...
def register_distributor():
    ...
    if not dist:
        ...
        dist = Distributor(code=c, name=d.get('name'), phone=d.get('phone'), email=email, is_verified=False, level=1)
...
def api_get_distributor_data():
    ...
    return jsonify({
        "name": dist.name, "code": dist.code, "discount": LEVELS[cur]['discount'], "commission_pct": LEVELS[cur]['commission'],
