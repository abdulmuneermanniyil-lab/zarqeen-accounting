...
app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
    "pool_pre_ping": True, "pool_recycle": 300, "pool_size": 2, "max_overflow": 1
}
...
class License(db.Model):
    ...
    # DYNAMIC PROPERTY: Calculate Expiry
    @property
...
def validate_license():
    ...
    if lic:
        ...
        lic.is_used = True; lic.used_at = datetime.utcnow(); lic.last_login_date = datetime.utcnow()
        lic.software_version = data.get('version', 'Unknown')
        db.session.commit()
...
def export_data(type):
    ...
    if type == 'licenses':
        cw.writerow(['Date', 'Key', 'Plan', 'Amount', 'Distributor', 'Status', 'Version', 'Last Login', 'Expiry Date'])
    ...
    output.headers["Content-Disposition"] = f"attachment; filename={type}.csv"; output.headers["Content-type"] = "text/csv"
    return output
